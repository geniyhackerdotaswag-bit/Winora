...
**Open issue — packaged activation.** `Add-AppxPackage -Register` from the built layout succeeds and Developer Mode is enabled, but activating the app creates a process that exits within a second: no window, no Windows Error Reporting entry, no managed exception, and no entry in the diagnostic sink. A module initializer confirmed the managed assembly loads with package identity and that `App` is never constructed, which places the failure in native XAML initialization. Building against 2.3.1 to match the bound framework did not change the behaviour. Until this is resolved, the apply path cannot be exercised, because `GlobalMutationLease` requires package identity by design. Probe, dry run, review, and rollback planning all work unpackaged.
